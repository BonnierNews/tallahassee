...
describe("cookies", () => {
  ...
  describe("navigation", () => {
    ...
    it("navigating with cookie header sets cookies on passed host", async () => {
      const browser = await new Browser(app, { headers: { host: "www.expressen.se" } });
    ...
    it("navigating with cookie header sets cookies on hostname", async () => {
      const browser = await new Browser(app, { headers: { "x-forwarded-host": "www.expressen.se" } });
    ...
    it("sends secure cookies with request with x-forwarded-proto https", async () => {
      const browser = await new Browser(app, { headers: { "x-forwarded-proto": "https" } });
    ...
    it("sends secure cookies with request to secure url", async () => {
      const browser = new Browser(app, { headers: { host: "www.expressen.se" } });
    ...
    it("assigns cookies to passed host header", async () => {
      const browser = new Browser(app);
      const page = await browser.navigateTo("/reply-with-cookies", { host: "www.expressen.se" });
      expect(page.document.body.textContent).to.not.be.ok;
    });

    it("response with set cookies assigns cookies to passed host header", async () => {
      const browser = new Browser(app);
      await browser.navigateTo("/setcookie", { host: "www.expressen.se" });
    ...
    it("host with port set cookies assigns cookies to hostname", async () => {
      const browser = new Browser(app);
      await browser.navigateTo("/setcookie", { host: "www.expressen.se:443" });
    ...
    it("response with set cookies assigns cookies to passed x-forwarded-host header", async () => {
      const browser = new Browser(app);
      await browser.navigateTo("/setcookie", { "x-forwarded-host": "www.expressen.se" });
  ...
  describe("document", () => {
    it("sets script cookies from response on document", async () => {
      const browser = await new Browser(app).navigateTo("/setcookie");
  ...
  describe("fetch", () => {
    it("forwards cookies", async () => {
      const browser = await new Browser(app).navigateTo("/", { cookie: "myCookie=singoalla" });
