...
function Document(resp, cookieJar) {
  const location = resp.location || getLocation(resp.url);
  const referrer = resp.headers ? resp.headers.get("referer") || "" : "";
  ...
  Object.defineProperty(document, "cookie", {
    get() {
      return cookieJar.getCookies({ path: location.pathname, script: true, domain: location.hostname}).toValueString();
    },
    set: (value) => {
