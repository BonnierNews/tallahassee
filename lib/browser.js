...
class Interceptor {
	constructor (browser, proxyOrigin) {
		this.browser = browser;
		this.interceptor = nock(proxyOrigin)
			.intercept(/./, /./);
	...
	clear () {
		nock.removeInterceptor(this.interceptor);
	}

	#forward (path, body, callback) {
		const { method, headers } = this.req;

		this.browser.request(path, { method, headers }, body)
