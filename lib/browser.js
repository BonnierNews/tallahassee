...
function Page(agentOrApp, cookieJar) {
  ...
  async function runScripts() {
    ...
    for (const scriptTag of dom.window.document.getElementsByTagName("script")) {
      ...
      const sourceFilePath = resources.resolve && resources.resolve(src);
      const entryModule = await Module(code, domContext, sourceFilePath);
...
async function Module (pendingCode, context, identifier) {
  ...
  const mod = new vm.SourceTextModule(code.toString(), { context, identifier });
...
function getSourceFilePath (scriptTag) {
  if (!scriptTag.src) return Promise.resolve();
