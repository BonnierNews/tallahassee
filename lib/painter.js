...
class Painter {
	...
	init (window) {
		...
		this.paint(window, { width: innerWidth, height: innerHeight }, null);
	...
	getLayout (source, relative) {
		console.log(source && source.tagName, source?.id);
		...
		if (compoundedStyles.height === 'auto') {
			const children = this.#getChildElements(source);
			console.log('calculating height', children.size);
			compoundedStyles.height = 0;
			for (const child of children) {
				compoundedStyles.height = this.getLayout(child).bottom;
			}
		}
		if (compoundedStyles.y === 'auto') {
			const siblings = this.#getChildElements(this.renderTree.get(source).parent);
			console.log('calculating y', source, siblings.length);

			compoundedStyles.y = 0;
			for (const sibling of siblings) {
				if (sibling === source) break;
				console.log(3);
				compoundedStyles.y = this.getLayout(sibling).bottom;
			}
		}
	...
	#getChildElements (element) {
		const children = new Set(
			this.renderTree.get(element).children
				.map(c => typeof c === 'string' ? Array.from(element.querySelectorAll(c)) : c)
				.flat()
		);

		for (const child of children) {
			this.#requireElementStyles(child);
			this.link(child, element);
		}
	...
	#requireElementStyles (element) {
		if (!this.elementStyles.has(element)) {
			...
			this.renderTree.set(element, { parent: this.window, children: [] });
		}
