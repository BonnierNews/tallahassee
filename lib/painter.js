...
module.exports = function Painter (options = {}) {
	...
	function init (_window) {
		...
		Object.defineProperties(window, {
			innerWidth: { get: GetDomRect('width', window) },
			innerHeight: { get: GetDomRect('height', window) },
			scrollX: { get: GetDomRect('scrollX', window) },
			scrollY: { get: GetDomRect('scrollY', window) },
			pageXOffset: { get: GetDomRect('scrollX', window) },
			pageYOffset: { get: GetDomRect('scrollY', window) },
			scroll: { value: scrollTo },
			scrollTo: { value: scrollTo },
			scrollBy: { value: scrollBy },
		});
		Object.defineProperties(window.Element.prototype, {
			getBoundingClientRect: { value: GetDomRect([ 'width', 'height', 'x', 'y', 'left', 'right', 'top', 'bottom' ]) },
			scrollIntoView: { value: scrollToElement },
		});
	...
	function GetDomRect (props, self) {
		return function getDomRect () {
			const layout = getLayout(self || this);
	...
	function getLayout (source) {
		...
		const compoundedLayout = getAncestors(source)
			...
			.reduce((layout, ancestorStyles) => {
				...
				return layout;
			}, { x: 0, y: 0, ...compoundedStyles });

		return Layout(compoundedLayout);
	}
