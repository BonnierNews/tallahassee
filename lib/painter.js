...
module.exports = function Painter (options = {}) {
	...
	function init (_window) {
		...
		Object.defineProperties(window.Element.prototype, {
			scrollWidth: { get: GetDomRect('scrollWidth') },
			scrollHeight: { get: GetDomRect('scrollHeight') },
	...
	function paint (target, styleChanges, parent) {
		if (typeof target === 'string') {
			...
			if (parent) elementParents.set(target, parent);
		}
	...
	function getAncestors (element) {
		...
		return ancestors.concat(window);
	}
};
...
function Layout (boundingBox = {}) {
	return Object.preventExtensions({
		...
		get scrollWidth () {
			return Math.max(this.width, boundingBox.scrollWidth || 0);
		},
		get scrollHeight () {
			return Math.max(this.height, boundingBox.scrollHeight || 0);
		},
