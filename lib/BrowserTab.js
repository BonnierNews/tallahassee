...
const { DragEvent } = require("./Events.js");
...
const dropPreventedSymbol = Symbol.for("dropPrevented");
...
module.exports = class BrowserTab {
  constructor(webPage, resp) {
    ...
    this[dropPreventedSymbol] = false;
  ...
  drag(x, y) {
    ...
    if (x instanceof HTMLElement) {
      ...
      overElement.addEventListener("dragover", (e) => {
        this[dropPreventedSymbol] = e.defaultPrevented;
      }, { once: true });
  ...
  dragEnd(element) {
    if (element instanceof HTMLElement) {
      if (!this[dropPreventedSymbol]) return;
