...
const respSymbol = Symbol.for("resp");
...
module.exports = class BrowserTab {
  constructor(webPage, resp) {
    ...
    this[respSymbol] = resp;
    this.location = getLocation(resp.url);
  ...
  get jar() {
    return this[webPageSymbol].agent.jar;
  }
  get response() {
    return this[respSymbol];
  }
  get _pending() {
  ...
  async focusIframe(element, src) {
    ...
    const srcUrl = makeAbsolute(this.window.location, src);
    const parsedUrl = new URL(srcUrl);
    const srcUri = parsedUrl.hostname === this.window.location.hostname ? element.getAttribute("src") : srcUrl;
