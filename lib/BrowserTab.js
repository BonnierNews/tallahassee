...
const vm = require("vm");
const domOverrides = require("./domOverrides.js");
...
module.exports = class BrowserTab {
  ...
  async load() {
    ...
    const dom = new JSDOM(await resp.text(), {
      ...
      cookieJar: this.jar,
      virtualConsole,
      runScripts: webPage.options.runScripts ? "dangerously" : undefined,
      pretendToBeVisual: true,
      beforeParse: (window) => {
        domOverrides(window, webPage.options);
      },
    });

    dom.window.fetch = new Fetch(webPage.fetch.bind(webPage));
