...
module.exports = class BrowserTab {
  ...
  async load() {
    ...
    document.addEventListener("click", this.onDocumentNavigate.bind(this));
  ...
  onDocumentNavigate(event) {
    const {tagName, href} = event.target;
    if (tagName === "A" && href) {
      this[pendingSymbol] = new Promise((resolve) => {
        if (event.defaultPrevented) return resolve(this);
        process.nextTick(this.onNavigate.bind(this), event, resolve);
      });
    }
  }
  async onNavigate(event, callback) {
    const {target} = event;
    const uri = target.href.startsWith(this.document.location.origin) ? target.pathname : target.href;
    callback(this[webPageSymbol].submit(uri));
  }
  navigateTo(...args) {
