...
function Element(document, $elm) {
  ...
  if (tagName === "form") {
    ...
    return (eventTarget = addFormFieldProxy(element));
  }
  ...
  function replaceChild(newChild, oldChild) {
    ...
    oldChild.$elm.replaceWith(newChild.nodeType === 3 ? newChild.textContent : newChild.outerHTML);
...
function addFormFieldProxy(element) {
  const nameHandler = {
    has(target, prop) {
