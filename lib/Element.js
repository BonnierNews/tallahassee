...
function Element(document, $elm) {
  ...
  //Object.defineProperty(element, "dataset", { get: () => {}, set: () => {}});
  ...
  function getDataset(_, prop) {
    if (!$elm || !$elm[0] || !$elm[0].attribs) return {};
    const {attribs} = $elm[0];
    const parsedDataset = Object.keys(attribs).reduce((acc, key) => {
