...
const nodeTypeSymbol = Symbol.for("nodeType");
...
class Element {
  ...
  get childNodes() {
    return this[elmSymbol].contents().map((_, e) => this.ownerDocument._getElement(this[dollarSymbol](e))).toArray();
  }
  get parentElement() {
    return this.ownerDocument._getElement(this.$elm.parent());
  }
  get parentNode() {
  ...
  set innerHTML(value) {
    ...
    if (this.tagName === "TEXTAREA") {
      this.value = this[elmSymbol].html();
    }
    this[emitterSymbol].emit("_insert");
  }
  get textContent() {
    return this.tagName === "SCRIPT" ? this[elmSymbol].html() : this[elmSymbol].text();
  }
  set textContent(value) {
    const response = this.tagName === "SCRIPT" ? this[elmSymbol].html(value) : this[elmSymbol].text(value);
    this[emitterSymbol].emit("_insert");
    return response;
  }
  get innerText() {
  ...
  get content() {
    ...
    return DocumentFragment(elementFactory, this);
  }
  get checked() {
  ...
  get options() {
    return this[elmSymbol].children("option").map((_, e) => this.ownerDocument._getElement(this[dollarSymbol](e))).toArray();
  }
  get selected() {
    return this[elmSymbol].prop("selected");
  }
  set selected(value) {
    const oldValue = this[elmSymbol].prop("selected");
    const $select = this[elmSymbol].parent("select");
    if (!$select.attr("multiple")) {
      if (value) this[elmSymbol].siblings("option").prop("selected", false);
    }

    this[elmSymbol].prop("selected", value);
  ...
  appendChild(childElement) {
    if (childElement instanceof DocumentFragment) {
      ...
      if (childElement.tagName === "SCRIPT") {
        vm.runInNewContext(childElement.innerText, this.ownerDocument.window);
      }
...
class Form extends Element {
  constructor(document, $elm) {
    ...
    const nameHandler = {
      ...
      get(target, prop) {
        ...
        const elements = element.$elm.find(`[name="${prop}"]`);

        if (elements.eq(0).attr("type") === "radio") return new RadioNodeList(element, `[name="${prop}"]`);
        return element.$elm.find(`[name="${prop}"]`).eq(0);
      }
    };
  ...
  get elements() {
    return this[elmSymbol].find("input,button,select,textarea").map((_, e) => this.ownerDocument._getElement(this[dollarSymbol](e))).toArray();
  }
  submit() {
  ...
  reset() {
    ...
    if ($inputs.length) {
      $inputs.each((idx, elm) => {
        this[dollarSymbol](elm).prop("checked", !!this[dollarSymbol](elm).attr("checked"));
      });
    }
    ...
    if ($options.length) {
      $options.each((idx, elm) => {
        this[dollarSymbol](elm).prop("selected", !!this[dollarSymbol](elm).attr("selected"));
      });
    }
...
function elementFactory(document, $elm) {
  const nodeType = $elm[0].nodeType;
  if (nodeType === 3) return new Text(document, $elm);
