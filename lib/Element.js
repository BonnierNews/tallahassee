...
function Element(document, $elm) {
  ...
  element._emitter.on("click", (e) => {
    if (element.parentElement && !e._propagationStopped) {
  ...
  function appendChild(childElement) {
    if (childElement.$elm) {
      ...
      $elm.append(childElement.textContent);
    }
    emitter.emit("_insert");
  }
  ...
  function contains(el) {
    return $elm.find(el.$elm).length > 0;
  }
  ...
  function emptyEvent() {
    ...
    Object.defineProperty(event, "_propagationStopped", {
